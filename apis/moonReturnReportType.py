...
def calculate_moon_return():
    try:
        ...
        write_to_file(f"Natal moon position: {natal_moon_position}")
        ...
        for line in result_output_min_newLine[6:60]:
            ...
            if len(splitbySpace) < 2:
                write_to_file(f"Error: Expected at least 2 parts after splitting line by 'Moon' but got {len(splitbySpace)}")
                continue
        ...
        for line in result_output_sec_newLine[6:60]:
            ...
            write_to_file(f"Most closest date value: {minSecMatch}")
            

        most_closest_date_sec_find = find_closest_key(most_closest_date_sec_dict, position_sec)
        write_to_file(f"Most closest date dictor: {most_closest_date_sec_dict}")
