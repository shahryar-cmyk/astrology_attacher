...
def parse_asteroid_output(asteroid_pholus_output):
    ...
    try:
        if len(lines) > 0:
            ...
            pattern3 = r"(?P<name>[a-zA-Z]+)\s+(?P<degree>\d+)\s+(?P<sign>\w+)\s+(?P<min>\d+)'(?P<sec>[\d.]+)"
            ...
            result[name] = {
                    ...
                    "position_sec": degree_match_min[1] if len(degree_match_min) > 1 else match3.group("sec"),    
            }
        else:
